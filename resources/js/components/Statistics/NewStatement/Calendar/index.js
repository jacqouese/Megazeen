...
function Calendar(props) {
  ...
  const handleYear = (action) => {
    ...
    else if (action == 'previous' && props.year > 1970) {
        props.setYear(props.year - 1);
    }
  }
  ...
  const handleMonth = (action) => {
    if (action == 'next') {
      ...
      //stop the user from going lower than January 1970
      if (props.month == 1 && props.year == 1970) {
        return
      }
    ...
    return (
      <div>
        <div className="calendar-body" style={{ marginBottom: 10 }} >
